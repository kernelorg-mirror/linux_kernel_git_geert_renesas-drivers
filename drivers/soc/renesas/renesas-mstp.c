...
static void renesas_test_mstp_clocks(unsigned int start_reg,
				     unsigned int start_bit)
{
	...
	for (r = start_reg; r < mstp->num_regs; r++) {
		...
		for (b = (r == start_reg ? start_bit : 31); b < 32; b--) {
...
static int renesas_mstp_setup(const struct renesas_mstp_info *info)
{
	...
	base = ioremap(info->mstp_base, info->mstp_size);
	if (!base) {
...
int __init renesas_disable_mstp_clocks(const struct renesas_mstp_info *info)
{
...
static const struct proc_ops mstp_proc_ops = {
	.proc_open	= mstp_proc_open,
	.proc_read	= seq_read,
	.proc_write	= mstp_write,
...
static ssize_t test_mstp_write(struct file *file, const char __user *user,
			       size_t size, loff_t *ppos)
{
	unsigned int len, nr, idx, bit;
	const char *key;
	char buf[32];
	int error;
	...
	renesas_test_mstp_clocks(idx, bit);

	return size;
}

static const struct proc_ops test_mstp_proc_ops = {
	.proc_open	= test_mstp_proc_open,
	.proc_read	= seq_read,
	.proc_write	= test_mstp_write,
	.proc_lseek	= seq_lseek,
	.proc_release	= single_release,
};
...
static int __init proc_mstp_init(void)
{
	...
	if (of_machine_is_compatible("renesas,r8a7740"))
	...
	if (of_machine_is_compatible("renesas,r8a7743"))
		error = renesas_mstp_setup(&r8a7791_mstp_info);
#endif
#ifdef CONFIG_ARCH_R8A7744
	if (of_machine_is_compatible("renesas,r8a7744"))
		error = renesas_mstp_setup(&r8a7791_mstp_info);
#endif
	...
	if (of_machine_is_compatible("renesas,r8a7795"))
		error = renesas_mstp_setup(&r8a7795_mstp_info);
#endif
#ifdef CONFIG_ARCH_R8A77960
	if (of_machine_is_compatible("renesas,r8a7796"))
		error = renesas_mstp_setup(&r8a77960_mstp_info);
#endif
#ifdef CONFIG_ARCH_R8A77961
	if (of_machine_is_compatible("renesas,r8a77961"))
		error = renesas_mstp_setup(&r8a77961_mstp_info);
#endif
#ifdef CONFIG_ARCH_R8A77965
	if (of_machine_is_compatible("renesas,r8a77965"))
		error = renesas_mstp_setup(&r8a77965_mstp_info);
#endif
#ifdef CONFIG_ARCH_R8A77990
	if (of_machine_is_compatible("renesas,r8a77990"))
		error = renesas_mstp_setup(&r8a77990_mstp_info);
#endif
	...
	proc_create("mstp", 0, NULL, &mstp_proc_ops);
	proc_create("mstp_test", 0, NULL, &test_mstp_proc_ops);
