...
struct mfis_info {
	...
	unsigned int mb_needs_distinct_channels:1;
};
					  ...
					  const struct of_phandle_args *sp)
{
	...
	if (priv->info->mb_needs_distinct_channels) {
		is_rx = chan_flags & MFIS_CHANNEL_RX;
		chan = mbox->chans + 2 * chan_num + is_rx;
	} else {
		is_rx = false; /* because channel is bidirectional */
	...
	per_chan_priv->reg = chan_num * 0x1000 + (tx_uses_eicr ^ is_rx) * 4;

	if (!priv->info->mb_needs_distinct_channels || is_rx) {
		char irqname[8];
...
static int mfis_mb_probe(struct mfis_priv *priv)
{
	...
	struct mbox_chan *chan;
	struct mbox_controller *mbox;
	int i, num_chan = priv->info->mb_num_channels;
...
//FIXME: doesn't work yet, still needs proper reg calculation
static const struct mfis_info mfis_info_r8a779g0 = {
	.unprotect_mask	= 0x0000ffff,
	.mb_num_channels = 12,
	.mb_needs_distinct_channels = true,
};
...
/* Hopefully for all of ES2 MFIS (except different num_chan):
static const struct mfis_info mfis_info_r8a78001 = {
	.unprotect_mask	= 0x000fffff,
	.mb_num_channels = 32,
};
*/
...
static const struct of_device_id mfis_mfd_of_match[] = {
	...
	/* for ES2
	{ .compatible = "renesas,r8a78001-mfis", .data = &mfis_info_..., },
	{ .compatible = "renesas,r8a78001-mfis-cross", .data = &mfis_info_..., },
	{ .compatible = "renesas,r8a78001-mfis-scp", .data = &mfis_info_..., },
	...
	*/
	{}
};
