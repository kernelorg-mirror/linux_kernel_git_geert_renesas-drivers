 ...
 *
 ****************************************************/
#if 0 /* This is not used from mailbox, but will be used from HW Spinlock */
...
static irqreturn_t mfis_mb_interrupt(int irq, void *data)
{
	...
	struct mfis_mb_chan *mchan = chan->con_priv;
					  ...
					  const struct of_phandle_args *sp)
{
	 ...
	 * check size
	 *
	 * priv->mnum_channels:	Total consumed channels == next channel position
	 * mbox->num_chans:	Max size of available channels
	 */
	if (priv->mnum_channels >= mbox->num_chans)
	 ...
	 */
	for (i = 0; i < priv->mnum_channels; i++)
	 ...
	 */
	mchan = priv->mchan + priv->mnum_channels;
	chan  = mbox->chans + priv->mnum_channels;
